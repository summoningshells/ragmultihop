...
class QueryExamples:
    ...
    SIMPLE_QUESTIONS = [
        ...
        "Quel est le prix du PG-C01?",
        "Comment fonctionne la garantie?",
        "Décris les batteries utilisées dans les produits"
    ]

    MULTI_HOP_QUESTIONS = [
        "Quels événements ont utilisé des produits vendus à Pollutec Paris?",
        "Quel est le CO2 total économisé par tous les déploiements de PG-M01?",
        "Liste les salons où on a vendu aux collectivités avec le revenu total",
        "Quels projets R&D visent à réduire les coûts des produits utilisés aux festivals?",
        "Quels produits ont été à la fois vendus et déployés?",
        "Dans quels salons le PG-M01 a-t-il été vendu?",
        "Quels sont les événements alimentés par des produits avec batteries Tesla?",
        "Combien de CO2 total a été économisé par tous les déploiements?"
    ]
...
if __name__ == "__main__":
    ...
    print("Questions SIMPLES (RAG classique):")
    for q in QueryExamples.SIMPLE_QUESTIONS[:3]:
        result = hybrid_rag.explain_routing(q)
        print(f"\nQ: {q}")
        print(f"Stratégie: {result['strategy']}")

    print("\n" + "="*60 + "\n")

    print("Questions MULTI-HOP (RAG hybride):")
    for q in QueryExamples.MULTI_HOP_QUESTIONS[:3]:
        result = hybrid_rag.explain_routing(q)
        print(f"\nQ: {q}")
        print(f"Stratégie: {result['strategy']}")
